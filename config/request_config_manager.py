...
class RequestConfigManager(object):
    ...
    def set_basic_url(self):
        self.basic_config[RequestConstants.JSON_URL] = BASE_URL
...
#         self.logger.info('http_request_body ' + str(self.http_request_body))
    # Convert dictionary to JSON string
        #self.http_request_body = json.dumps(self.http_request_body, indent=4)
        self.logger.info('http_request_body ' + str(self.http_request_body))
    ...
    def set_http_request_body_with_user_list(self, user):
        self.http_request_body = user.user_list
        print(f'body body{self.http_request_body}')

    def set_http_request_body_with_order_details(self, order):
        self.http_request_body[RequestConstants.JSON_ID] = order.order_id
        self.http_request_body[RequestConstants.JSON_ORDER_QUANTITY] = order.quantity
        self.http_request_body[RequestConstants.JSON_SHIP_DATE] = order.ship_date
        self.http_request_body[RequestConstants.JSON_ORDER_STATUS] = order.status
        self.http_request_body[RequestConstants.JSON_ORDER_COMPLETE] = order.complete
        self.http_request_body[RequestConstants.JSON_PET_ID] = order.pet_id
